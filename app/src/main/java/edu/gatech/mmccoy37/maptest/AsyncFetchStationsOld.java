 ...
 * @author matt
 */
public class AsyncFetchStationsOld extends AsyncTask<String, Void, Integer> {
    ...
    public AsyncFetchStationsOld() {
        super();
    }

    public AsyncFetchStationsOld(Location loc) {
    ...
    public void setStationMap(HashMap<Location, StationItem> stationMap) {
        this.stationMap = stationMap;
    }

    private HashMap<Location, CityItem> parseCities(Location center, String json, float dist) {
    ...
    protected Integer doInBackground(String... params) {
        ...
        try {
            ...
            if (statusCode == 200) {
                ...
                this.cityMap = parseCities(center, response.toString(), 300f * ONE_MILE_IN_METERS);
                if (getCityMap() == null) {
                    result = 0;
                }
            } else {
            ...
            //get all stations for these local cities
            for (CityItem i : cityMap.values()) {
                url = new URL(API_URL_BASE + i.getHref());
                Log.d("API_TAG", "city url: " + url.toString());
                urlConnection = (HttpURLConnection) url.openConnection();
                ...
                if (statusCode == 200) {
                    ...
                    //add citites stations to map if they are nearby
                    float distance = ONE_MILE_IN_METERS * 200f;
                    parseStations(center, response.toString(), 300f * ONE_MILE_IN_METERS);
                }
            }
            if (getStationMap() == null) {
                result = 0;
            } else {
                return 1;
            }
